...
//	private static boolean saveToCommonCsv(String hotelDetailsResponse, String cityName, FileWriter commonWriter,
//			boolean isHeaderWritten) throws IOException {
//		Gson gson = new Gson();
...
//			}
//			StringBuffer res = hotelData(cityName, hotels);
//
...
//	}
//
//	public static StringBuffer hotelData(String cityName, JsonArray hotels) {
...
//            String state = null;
//            if (hotel.has("State")) {
//                state = hotel.get("State").getAsString();
//            }
//            appendColumnValue(entry, state);
//
//            String updated = null;
...
////            if (hotel.has("Description")) {
////                description = hotel.get("Description").getAsString();
////            }
//            appendColumnValue(entry, description);
//
//            String amenities = null;
...
//
//            String checkin = null;
//            if (hotel.has("CheckIn")) {
//                checkin = hotel.get("CheckIn").getAsString();
//            }
//            appendColumnValue(entry, checkin);
//
//            String checkout = null;
//            if (hotel.has("CheckOut")) {
//                checkout = hotel.get("CheckOut").getAsString();
//            }
//            appendColumnValue(entry, checkout);
//
//            String reviewRating = null;
...
public class App {
	...
	public static void main(String[] args) {
		...
		try {
			...
			for (int i = 0; i < cityList.size(); i++) {
				...
				JsonArray hotels = hotelDetailsJson.has("Hotels") ? hotelDetailsJson.getAsJsonArray("Hotels") : new JsonArray();

				// Check if hotels array is not empty
				if (hotels.size() > 0) {
					// Save hotel details to the individual city CSV
					saveHotelsToCsv(hotelDetailsResponse, cityName,true);

					// Save hotel details to the common CSV
					saveToCommonCsv(hotelDetailsResponse, cityName, commonWriter, isHeaderWrittenForCommonCsv,true);
					isHeaderWrittenForCommonCsv = true; // After the first city, the header will be written
				} else {
					...
					saveHotelsToCsv(hotelDetailsResponse, cityName,false);

					// Save hotel details to the common CSV
					saveToCommonCsv(hotelDetailsResponse, cityName, commonWriter, isHeaderWrittenForCommonCsv,false);
					isHeaderWrittenForCommonCsv = true;
				}
	...
	// Save hotel data to the common CSV file
	private static boolean saveToCommonCsv(String hotelDetailsResponse, String cityName, FileWriter commonWriter,
			boolean isHeaderWritten,boolean isDetailResponse) throws IOException {
		Gson gson = new Gson();
		JsonObject hotelDetailsJson = gson.fromJson(hotelDetailsResponse, JsonObject.class);
		...
		if (hotelDetailsJson.has("Hotels") && hotelDetailsJson.get("Hotels").isJsonArray()) {
			...
			for (int i = 0; i < hotels.size(); i++) {
				JsonObject hotel = hotels.get(i).getAsJsonObject();
				StringBuffer res = hotelData(cityName, hotel,isDetailResponse);
	...
	// Save hotel data to the individual city's CSV
	private static void saveHotelsToCsv(String hotelDetailsResponse, String cityName,boolean isDetailResponse) {
		try {
			Gson gson = new Gson();
			JsonObject hotelDetailsJson = gson.fromJson(hotelDetailsResponse, JsonObject.class);
	...
	public static StringBuffer hotelData(String cityName, JsonObject hotel,boolean isDetailResponse) {
		StringBuffer entry = new StringBuffer();
		...
		String latitude = null;
		String longitude = null;
		if(isDetailResponse==false)
		{
			if(hotel.has("Latitude"))
			{
				latitude = hotel.get("Latitude").getAsString();
			}
			if(hotel.has("Longitude"))
			{
				longitude = hotel.get("Longitude").getAsString();
			}
			appendColumnValue(entry, latitude);
			appendColumnValue(entry, longitude);
